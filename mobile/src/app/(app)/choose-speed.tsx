      ...
      <View style={styles.container}>
        <View style={styles.header}>
          <TouchableOpacity onPress={() => router.back()} style={styles.back}>
            <Text style={styles.backText}>{t.speed.back}</Text>
          </TouchableOpacity>
          <BalanceBadge balanceUsdDisplay={balance_usd_display} loading={loading} />
        </View>
        <Text style={styles.title}>{t.speed.title}</Text>
        <Text style={styles.subtitle}>{subtitleText}</Text>
        <TouchableOpacity style={styles.option} onPress={() => handleSelect('standard')} activeOpacity={0.85}>
          <View style={styles.optionHeader}>
            <View>
              <Text style={styles.optionTitle}>{t.speed.standardTitle}</Text>
              <Text style={styles.optionDesc}>{t.speed.standardDesc}</Text>
            </View>
            <View style={styles.priceBadge}>
              <Text style={styles.priceText}>${(standardPricing.credits_per_second * 60 * standardPricing.usd_per_credit).toFixed(3)}/min</Text>
            </View>
          </View>
          {standardEstimate && <Text style={styles.estimate}>≈ {standardEstimate.usd}</Text>}
        </TouchableOpacity>
        <TouchableOpacity style={[styles.option, styles.optionFast]} onPress={() => handleSelect('chirp')} activeOpacity={0.85}>
          <View style={styles.optionHeader}>
            <View>
              <Text style={styles.optionTitle}>{t.speed.fastTitle}</Text>
              <Text style={styles.optionDesc}>{t.speed.fastDesc}</Text>
            </View>
            <View style={[styles.priceBadge, styles.priceBadgeFast]}>
              <Text style={styles.priceText}>${(chirpPricing.credits_per_second * 60 * chirpPricing.usd_per_credit).toFixed(3)}/min</Text>
            </View>
          </View>
          {chirpEstimate && <Text style={styles.estimate}>≈ {chirpEstimate.usd}</Text>}
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  option: { backgroundColor: Colors.surface, borderRadius: BorderRadius.lg, padding: Spacing.lg, borderWidth: 1.5, borderColor: Colors.border, gap: Spacing.sm },
  optionFast: { borderColor: Colors.accent, backgroundColor: Colors.accentLight },
  ...
  priceBadge: { backgroundColor: Colors.border, paddingHorizontal: Spacing.sm, paddingVertical: Spacing.xs, borderRadius: BorderRadius.full },
  priceBadgeFast: { backgroundColor: Colors.accent },
  priceText: { fontSize: 13, fontWeight: '700', color: Colors.primary },
  estimate: { ...Typography.h3, color: Colors.accent },
});

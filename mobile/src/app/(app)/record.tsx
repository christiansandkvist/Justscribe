...
export default function RecordScreen() {
  ...
  useEffect(() => {
    if (error) {
      if (error === 'insufficient_balance') {
        Alert.alert(t.record.insufficientBalance, t.record.insufficientBalanceMsg, [{ text: t.record.ok, onPress: () => router.replace('/(app)/home') }]);
      } else {
  ...
  async function handlePress() {
    if (recorder.state === 'idle') {
      ...
      if (result) {
        router.replace({ pathname: '/(app)/result', params: { transcript: result.transcript, duration_seconds: String(result.duration_seconds), credits_charged: String(result.credits_charged), new_balance: String(result.new_balance) } });
      }
    }
  }
    ...
    <SafeAreaView style={styles.safe}>
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.modelBadge}>{model === 'chirp' ? t.record.fastBadge : t.record.standardBadge}</Text>
        </View>
        <View style={styles.center}>
          <Text style={styles.timer}>{isProcessing ? (status === 'uploading' ? t.record.uploading : t.record.transcribing) : formatDuration(recorder.elapsedSeconds)}</Text>
          {recorder.state === 'recording' && <Text style={styles.liveCost}>≈ {liveEstimate.usd}</Text>}
        </View>
        {!isProcessing && (
        ...
        {isProcessing && (
          <View style={styles.processingArea}>
            <Text style={styles.processingText}>{status === 'uploading' ? t.record.uploading : t.record.transcribing}</Text>
          </View>
        )}
        <Text style={styles.cancelText} onPress={() => { recorder.reset(); router.back(); }}>{t.record.cancel}</Text>
      </View>
    </SafeAreaView>
  );
}
